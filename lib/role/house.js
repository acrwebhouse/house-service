...
function watermarkJob(photo,folderPath,callback){
    const originPath = path.join(folderPath,'origin');
    utilsFile.mkdir(origin,(result)=>{
        if(result == true){
            for(let i = 0;i<photo.length;i++){
                const fileName = path.basename(photo[i])
                filesCopy.push({
                    orgPath : path.join(folderPath,fileName),
            ...
            utilsFile.copyFiles(filesCopy,(err,result)=>{
                console.log('=====copyFiles==result==',result)
                if(result){
                    sendWatermarkAddJob(originPath,folderPath,callback)
                }else{
