...
const reserveHouse = require('./reserveHouse');
const errorMessage = require('../utils/error').message;

function removeOwnerDetailData(ownerDetail){
    if(utilsValue.isValid(ownerDetail)){
        delete ownerDetail.password
    }
}
...
function getHouses(start,count,timeSort,priceSort,pingSort,minPrice,maxPrice,minPing,maxPing,minRoom,maxRoom,minFloor,maxFloor,owner,buildingType,typeOfRental,city,area,parking,pet,manager,garbage,smoke,cook,textQuery,belongType,belongId,isDelete,callback) {
    ...
    url = url + '&&isShowTransaction=true'
        ...
        if(result === true){
            ...
            editHouse(id,name,city,area,owner,address,houseNumber,totalFloor,floor,floor2,room,price,hostName,hostPhone,hostGender,houseConfig,ping,parking,traffic,life,educate,saleType,saleInfo,photo,annex,remark,belongType,belongId,isRoofAnnex,(result,data)=>{
                if(result === true){
                    ...
                    if(orgHouse.owner !== owner){
                        reserveHouse.transferReserveHouse(id,orgHouse.owner,owner,()=>{})
                    }
                    callback(true,data)
                }else{
                    callback(false,data)
...
function getLatLon(address,callback){
    const googleCompareStr = config['google-map-server'].googleCompareStr
    const url = googleCompareStr+'/'+encodeURIComponent(address)
    const method = 'GET';
    const headers = {};
    const result = {
        lat : '',
    ...
    httpRequest.sendGetRequest(url, headers, method, (error, body) => {
        if(error){
            ...
            let subStr = body.substring(body.indexOf(googleCompareStr))
            
            subStr = subStr.substring(subStr.indexOf('@'))
            const lat = subStr.substring(1,subStr.indexOf(','))
            subStr = subStr.substring(subStr.indexOf(',')+1)
            const lon = subStr.substring(0,subStr.indexOf(','))
            if(utilsValue.isNumber(lat)&&utilsValue.isNumber(lon)){
                result.lat = lat
                result.lon = lon
            }
            callback(result)
...
function getHouse(id,isDelete,callback){
    let url = config['house-basic-server'].location+'/'+config['house-basic-server'].restApi.getHouse + '?id='+id+'&isDelete='+isDelete + '&&isShowTransaction=true'
    const method = 'GET';
    const headers = {};
    httpRequest.sendGetRequest(url, headers, method, (error, body) => {
        if (error) {
                ...
                if(body.status  == true && body.data && body.data.owner){
                    ...
                    user.getUserById(owner,isDelete,(result,msg)=>{
                        if(result){
                            ...
                            removeOwnerDetailData(body.data.ownerDetail)
                            const houseNumber = body.data.houseNumber
                            let address = body.data.address 
                            if(utilsValue.isValid(houseNumber)){
                                if(utilsValue.isValid(houseNumber.lane)){
                                    address += houseNumber.lane +'巷'
                                }
                                if(utilsValue.isValid(houseNumber.alley)){
                                    address += houseNumber.alley +'弄'
                                }
                                if(utilsValue.isValid(houseNumber.number1)){
                                    address += houseNumber.number1 +'號'
                                }
                            }
                            getLatLon(address,(postion)=>{
                                body.data.lat = postion.lat
                                body.data.lon = postion.lon
...
function sendWatermarkAddJob(input,output,companyId,callback) {
    ...
    const job = {
        ...
        info : {
            input:{
                path : path.join('house',input)
            },
            output : path.join('house',output)
        }
    }
    if(utilsValue.isValid(companyId)){
        job.info.input.watermarkPath = path.join('company',companyId,'image','watermark.png')
    }else{
        job.info.input.watermarkPath = 'watermark.png'
    }
...
function watermarkJob(photo,folderPath,companyId,callback){
    ...
    utilsFile.mkdir(originPath,(result)=>{
        if(result == true){
            ...
            utilsFile.copyFiles(filesCopy,(err,result)=>{
                if(result == true){
                    sendWatermarkAddJob(originPath,photoPath,companyId,callback)
                }else{
