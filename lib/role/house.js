...
function moveImage(files,folderName,destFolder,callback){
    if(files.length <= 0){
        callback(true)
    }else{
        const uploadDir = path.join(__dirname, '..', '..', 'uploadTmp');
        ...
        utilsFile.moveImagesAndResize(filesMove,(err,result)=>{
            callback(result)
        })
    }
}
...
function moveFiles(photo,annex,destFolder,callback){
    ...
    moveImage(photo,photoFolder,destFolder,(result)=>{
        if(result){
