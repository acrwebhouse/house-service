...
function moveImageAndResize(input,output,callback){
    Jimp.read(input, (err, readImage) => {
        if(err){
            callback(err,false)
        }else{
            const resizeValue = config.resizeImage;
            readImage
                .resize(resizeValue, Jimp.AUTO) // resize
                .write(output); // save  
            callback(err,true)
        }
    })
...
function moveImagesAndResize(filePathArray, callback) {
    let cbs = [];
    for(let i =0 ;i<filePathArray.length; i++){
        cbs.push(function(callback) {
            moveImageAndResize(filePathArray[i].orgPath,filePathArray[i].distPath, callback)
        })
    }
...
exports.moveImagesAndResize = moveImagesAndResize