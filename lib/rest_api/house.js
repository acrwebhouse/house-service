exports.on = function(app) {
    ...
    app.post(preRestApi + '/uploadHousePhoto', function(req, res) {
        /* 
        #swagger.security = [{
               "apiKeyAuth": []
        }] 
            ...
            if(result == true){
                utilsFile.getUploadFiles(req, uploadDir,'photo',(result,files)=>{
                    ...
                    for(let i = 0 ;i<files.length;i++){
                        ...
                        let localPath = path.join(dir,files[i].originalname)
                        const item = {
                            orgPath,
                            distPath
                        }
                        filePath.push(item)
                        let orgExtname = path.extname(localPath)
                        let extname = orgExtname.toLowerCase()
                        if(extname === '.heic'){
                            localPath = path.join(path.dirname(localPath),path.basename(localPath,orgExtname)+'.jpg')
                        }
                    ...
                    if(filePath.length==0){
                        ...
                        utilsFile.moveFiles(filePath,(err,result)=>{
                            if(result == true){
                                ...
                                .catch(error => {
                                    response.status = false
                                    response.data = 'heic file convert fail'
                                    res.send(response)
                                });
                            }else{
                                response.status = false
    ...
    app.post(preRestApi + '/uploadHouseAnnex', function(req, res) {
        /* 
        #swagger.security = [{
               "apiKeyAuth": []
        }] 
